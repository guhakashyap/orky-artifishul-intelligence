...
NOW WIF MASKS SO DA ORKS CAN'T CHEAT BY LOOKIN' AHEAD!
AND WIF ALL DA DETAILED ORKY COMMENTS SO EVEN DA DUMBEST GROT CAN UNDERSTAND!
...
class OrkAttentionHead(nn.Module):
   ...
   def do_da_orky_finkin(self, da_wordz, da_orky_mask=None):
       ...
       # STEP 4: NEW ORKY BIT - DA NO-CHEATIN' MASK!
       # If dere's a mask, we use it to stop da Orks from lookin' ahead!
       # Dis is like puttin' blinders on da Orks so dey only pay attention to words dey've already seen
       # masked_fill: Dis replaces certain values in da tensor wif a new value
       # da_orky_mask == 0: Dis finds all da places where da mask is 0 (where we don't want da Ork to look)
       # float('-inf'): Dis replaces those places wif negative infinity (a really big negative number)
       # Why we need 'em (Orky): We need to stop da Orks from cheatin' by lookin' at future words!
       # Why we need 'em (Humie): Causal masking prevents the model from attending to future tokens,
       # which is essential for autoregressive language modeling and prevents data leakage
       # Dis is like tellin' da Ork: "DON'T LOOK AT DESE WORDS - DEY HAVEN'T HAPPENED YET!"
       if da_orky_mask is not None:
           da_orky_scores = da_orky_scores.masked_fill(da_orky_mask == 0, float('-inf'))
      
       # STEP 5: Da Ork turns da scores into probabilities
       ...
       # If we used da mask, da future words will have -inf scores, which become 0 after softmax!
       da_orky_attention_weights = F.softmax(da_orky_scores, dim=-1)
      
       # STEP 6: Da Ork combines all da information based on his attention
...
class MultiHeadOrkAttention(nn.Module):
   ...
   def do_da_mob_finkin(self, da_wordz, da_orky_mask=None):
       """
       ALL DA ORK HEADS WORK TOGETHER TO SOLVE DA PROBLEM!
      
       da_wordz: Da words dat all da Ork heads need to look at
       da_orky_mask: Da no-cheatin' mask we pass to each head so dey don't cheat!
       ...
       for ork_head in self.da_orky_heads:
           ...
           # We pass da mask to each head so dey all follow da same no-cheatin' rules!
           da_head_answer, da_head_attention = ork_head.do_da_orky_finkin(da_wordz, da_orky_mask)
...
class OrkyTransformerBlock(nn.Module):
   ...
   def do_da_complete_orky_processin(self, da_wordz, da_orky_mask=None):
       """
       DA COMPLETE ORK PROCESSING PIPELINE!
      
       da_wordz: Da words dat need to be processed by da Ork transformer block
       da_orky_mask: Da no-cheatin' mask for da attention mob
       ...
       # We pass da mask to da attention so da Orks don't cheat!
       da_attention_output, da_attention_weights = self.da_orky_attention.do_da_mob_finkin(da_wordz, da_orky_mask)
...
class OrkyTransformer(nn.Module):
   ...
   def do_da_complete_orky_transfo_ma(self, da_wordz, da_orky_mask=None):
       """
       DA COMPLETE ORKY TRANSFO'MA' IN ACTION!
      
       da_wordz: Da input words dat need to be processed (batch_size, seq_len)
       da_orky_mask: Da no-cheatin' mask we pass all da way down
       ...
       for orky_transformer_block in self.da_orky_transformer_blocks:
           # We pass da mask to each block so da Orks don't cheat!
           da_orky_embedded_words, da_attention_weights = orky_transformer_block.do_da_complete_orky_processin(da_orky_embedded_words, da_orky_mask)
...
def create_da_orky_no_peek_mask(da_seq_len):
   """
   CREATE DA NO-PEEK MASK FOR DA ORKS!
   
   Dis creates a mask dat stops da Orks from lookin' at future words.
   It's like puttin' blinders on 'em so dey only pay attention to words
   dey've already seen!
   
   DIS IS ESSENTIAL FOR AUTOREGRESSIVE LANGUAGE MODELING!
   """
   # torch.tril: Dis creates a lower triangular matrix (all 1s below da diagonal, 0s above)
   # torch.ones: Creates a matrix filled wif 1s
   # Why we need 'em (Orky): We need a triangle shape so da Orks can only look at past words!
   # Why we need 'em (Humie): Lower triangular matrix ensures causal attention - each position
   # can only attend to itself and previous positions, preventing information leakage from future tokens
   # Dis is like givin' each Ork a pair of blinders dat only let 'em see backwards!
   da_orky_mask = torch.tril(torch.ones(da_seq_len, da_seq_len))
   
   # Add batch and head dimensions so it works wif our multi-head attention
   # view(1, 1, da_seq_len, da_seq_len): Reshapes da mask to (1, 1, seq_len, seq_len)
   # Why we need 'em (Orky): We need to make it da right shape for all da Ork heads!
   # Why we need 'em (Humie): The mask needs to be broadcastable across batch and head dimensions
   # Dis makes our mask work wif da attention mechanism's expected input shape
   da_orky_mask = da_orky_mask.view(1, 1, da_seq_len, da_seq_len)
...
def demonstrate_da_orky_transfo_ma():
   ...
   NOW WIF MASKS SO DA ORKS CAN'T CHEAT BY LOOKIN' AHEAD!
   """
   print("WAAAGH! STARTIN' DA ORKY TRANSFO'MA' v1.3 DEMONSTRATION!")
   print("=" * 70)
   ...
   print(f"- NOW WIF MASKS SO DA ORKS CAN'T CHEAT!")
   print()
   ...
   # Create da no-peek mask so da Orks can't cheat!
   da_seq_len = da_orky_input_tensor.size(1)
   da_no_peek_mask = create_da_orky_no_peek_mask(da_seq_len)
  
   print("Input Ork sentence:", " ".join(da_orky_sentence))
   print("Input tensor shape:", da_orky_input_tensor.shape)
   print("Mask shape:", da_no_peek_mask.shape)
   print("Mask preview (first 5x5):")
   print(da_no_peek_mask[0, 0, :5, :5].numpy())
   print()
  
   # Run da Orky Transformer WIF DA MASK!
   print("RUNNIN' DA ORKY TRANSFO'MA' WIF DA NO-CHEATIN' MASK...")
   with torch.no_grad():
       da_orky_output, all_da_orky_attention_weights = da_orky_transformer.do_da_complete_orky_transfo_ma(da_orky_input_tensor, da_no_peek_mask)
   ...
   # Show what da Orks are payin' attention to (now wif proper masking!)
   print("DA ORKS ARE PAYIN' ATTENTION TO (WIF NO CHEATIN'!):")
   print("-" * 60)
   ...
   print("WAAAGH! DA ORKY TRANSFO'MA' v1.3 IS WORKIN' PERFECTLY!")
   print("Da Orks are lookin' at words and figurin' out which ones matter!")
   print("AND DEY CAN'T CHEAT BY LOOKIN' AHEAD - PROPPA' ORKY DISCIPLINE!")
   print("NOW WIF MASKS AND ALL DA DETAILED ORKY COMMENTS!")
   print("=" * 70)
